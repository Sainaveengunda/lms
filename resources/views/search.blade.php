...
<div class="flex justify-center flex-cols m-6 justify-around">   
   @foreach($products as $product)
            <div class="max-auto rounded overflow-hidden shadow-lg">
                <img class="w-full" src="{{  Storage::url($product->course_image)}}" type="jpg/png/jpeg" alt="Sorry Not displayed" style="height: 200px; width: 350px">
                <div class="px-6 py-4">
                <a href="/course/{{$product->id}}" class="font-bold text-xl mb-2">{{$product->course_title}}</a>
                     <p class="text-gray-700 text-base">{{ $product->course_description }}</p>
                 </div>
