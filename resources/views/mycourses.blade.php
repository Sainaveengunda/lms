...
<div>
<div class="flex justify-center flex-cols m-6 justify-around">   
   @foreach(auth()->user()->follows as $course)
            
           
            <div class="max-auto rounded overflow-hidden shadow-lg">
                <img class="w-full" src="{{  Storage::url($course->course_image)}}" type="jpg/png/jpeg" alt="Sorry Not displayed" style="height: 200px; width: 350px">
                <div class="px-6 py-4">
                <a href="/enrolled/{{$course->id}}" class="font-bold text-xl mb-2">{{$course->course_title}}</a>
                     <p class="text-gray-700 text-base">{{ $course->course_description }}</p>
                 </div>
